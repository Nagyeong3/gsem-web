...
현재 기준은 `release/prototype-v7`을 준비하는 인터랙티브 프로토타입이다. 프론트는 React·TypeScript·Vite, 백엔드는 FastAPI로 구성하며 JSON 인메모리 Repository와 실제 HTTP 통신까지 구현되어 있다. 실제 MS Access, 인증·SSO, 운영 권한, 쓰기 트랜잭션 및 상세 업무 규칙은 아직 확정되지 않았다.
...
3. `docs/handoff/confirmed-decisions.md`
4. `docs/handoff/next-actions.md`
5. 작업과 직접 관련된 상세 문서

`docs/handoff` 문서를 현재 상태의 정본으로 사용한다. 과거 V2~V6 릴리즈 문서는 당시 기록이며 현재 구현과 충돌할 경우 정본보다 우선하지 않는다.
...
현재 인터랙티브 프로토타입은 다음 여섯 화면으로 구성한다.
...
영속 데이터 모델은 아직 확정되지 않았다. 화면 필드만 보고 운영용 데이터베이스 스키마를 만들지 않는다. FastAPI의 인메모리 Repository는 프로토타입 실행과 API 계약 검증용이며 운영 DB가 아니다.
